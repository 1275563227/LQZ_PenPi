...
public class NotificationUtil {
     ...
     * 普通的Notification
     */
    public void postNotification(String alert, String title, String content) {
...
//        builder.setTicker(alert);               // 第一次提示消失的时候显示在通知栏上的
        builder.setDefaults(Notification.DEFAULT_ALL);
//        builder.setAutoCancel(false);           // 自己维护通知的消失
//        builder.setOngoing(false);              // 设置为ture，表示它为一个正在进行的通知。简单的说，当为ture时，不可以被侧滑消失。
//        builder.setNumber(20);                  // 在TextView的右方显示的数字，可放大图片看，在最右侧。
