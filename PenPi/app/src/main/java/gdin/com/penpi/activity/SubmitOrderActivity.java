...
public class SubmitOrderActivity extends AppCompatActivity {
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == 0x123) {
                // 设置show组件显示服务器响应
                et_start.setText(order_response.getStart_place());
                et_end.setText(order_response.getEnd_place());
    ...
    public void Submit_to_Sever(View view) {
        ...
        new Thread() {
            @Override
            public void run() {
                response = ClientUtils.sendPost("http://192.168.1.118:8080/Demo/servlet/testClientServlet", order);
                if (response != null) {
